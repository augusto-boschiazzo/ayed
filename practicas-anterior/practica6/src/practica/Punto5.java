...
public class Punto5 {
	...
	public ListaGenerica<String> devolverCaminoSinCargar(String ciudad1, String ciudad2, int nafta) {
		...
		if (i != -1) {
			dfsSinCargar(i, marca, lista, camino, ciudad2, nafta);
		}
		
		return camino;
	}
	
	public void dfsSinCargar(int i, boolean[] marca, ListaEnlazadaGenerica<String> lista, ListaEnlazadaGenerica<String> camino, String ciudad2, int nafta) {
		marca[i] = true;
		Vertice<String> v = mapaCiudades.listaDeVertices().elemento(i);
		if(v.dato() == ciudad2) {
			clonar(lista, camino);
		...
		if (camino.esVacia()) {
			...
			while (!ady.fin() && camino.esVacia()) {
				...
				if (!marca[j]) {
					...
					if(nafta >= 0) {
						lista.agregarFinal(destino.dato());
						dfsSinCargar(j, marca, lista, camino, ciudad2, nafta);
						lista.eliminarEn(lista.tamanio());
					}
					nafta += arista.peso();
				}
	...
	public ListaGenerica<String> devolverCaminoMenorCarga(String ciudad1, String ciudad2, int nafta) {
		boolean[] marca = new boolean[mapaCiudades.listaDeVertices().tamanio() +1];
		ListaEnlazadaGenerica<String> lista = new ListaEnlazadaGenerica<String>();
		ListaEnlazadaGenerica<String> camino = new ListaEnlazadaGenerica<String>();
			ListaGenerica<Vertice<String>> aux = mapaCiudades.listaDeVertices();
		...
		if (i != -1) {
			dfsMenorCarga(i, marca, lista, camino, ciudad2, nafta);
		}
		
		return camino;
	}
	
	public void dfsMenorCarga(int i, boolean[] marca, ListaEnlazadaGenerica<String> lista, ListaEnlazadaGenerica<String> camino, String ciudad2, int nafta) {
		marca[i] = true;
		Vertice<String> v = mapaCiudades.listaDeVertices().elemento(i);
		if(v.dato() == ciudad2) {
			clonar(lista, camino);
		...
		if (camino.esVacia()) {
			...
			while (!ady.fin() && camino.esVacia()) {
				...
				if (!marca[j]) {
					...
					if(nafta >= 0) {
						lista.agregarFinal(destino.dato());
						dfsMenorCarga(j, marca, lista, camino, ciudad2, nafta);
						lista.eliminarEn(lista.tamanio());
					}
					nafta += arista.peso();
				}
